...
class UpdateAtributoRequest extends FormRequest
{
    ...
    {
        $atributoId = $this->route('atributo') ? $this->route('atributo')->id : null;
        ...
        return [
            'nombre' => [
                ...
                'string',
                'max:255',
                Rule::unique('atributos', 'nombre')->ignore($atributoId),
            ],
            'tipo' => ['sometimes', 'string', Rule::in(['texto', 'color', 'numero', 'tamaño', 'booleano'])],
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->has('filtrable') && !is_bool($this->filtrable)) {
            $this->merge([
                'filtrable' => filter_var($this->filtrable, FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) ?? $this->route('atributo')->filtrable,
            ]);
        }
        if ($this->has('visible') && !is_bool($this->visible)) {
            $this->merge([
                'visible' => filter_var($this->visible, FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) ?? $this->route('atributo')->visible,
            ]);
        }
    }
