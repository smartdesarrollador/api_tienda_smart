...
class UpdateValorAtributoRequest extends FormRequest
{
    ...
    {
        $valorAtributoId = $this->route('valor_atributo');
        ...
        return [
            ...
            'valor' => [
                ...
                Rule::unique('valores_atributo')->where(function ($query) {
                    return $query->where('atributo_id', $this->atributo_id ?? $this->valorAtributo->atributo_id);
                })->ignore($valorAtributoId)
            ],
            'codigo' => [
                'nullable',
                'string',
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            $atributoId = $this->atributo_id ?? $this->valorAtributo->atributo_id;
            $atributo = Atributo::find($atributoId);
            
            if ($atributo && $this->has('valor')) {
                // Validaciones específicas según el tipo de atributo
                $this->validateByAttributeType($validator, $atributo);
            }

            // Validar que no se pueda cambiar el atributo si el valor ya está en uso
            if ($this->has('atributo_id') && $this->atributo_id != $this->valorAtributo->atributo_id) {
                if ($this->valorAtributo->variaciones()->exists()) {
                    $validator->errors()->add('atributo_id', 'No se puede cambiar el atributo porque este valor está siendo usado por productos.');
                }
            }
        });
    }
